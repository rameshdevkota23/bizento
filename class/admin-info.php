...
class Bizento_Admin_Info
{
    ...
    {
        add_theme_page(
            esc_html__('bizento Theme', 'bizento'),
            esc_html__('bizento Theme', 'bizento'),
            'edit_theme_options',
            'bizento',
    ...
    public function check_rt_easy_builder_freemius()
    {
        if (function_exists('RT_Easy_Builder\rt_freemius') && ! RT_Easy_Builder\rt_freemius()->is_not_paying()) {
            ...
            add_action('wp_ajax_dismiss_bizento_notification', array($this, 'dismiss_notification_ajax_handler'));
        }
    }
    ...
    {
        delete_transient('bizento_notification_dismissed');
    }
    ...
    public function compatible_check()
    {
        ...
        $dismissed = get_option('bizento_notification_dismissed');
        if (!$dismissed) {
    ...
    public function dismiss_notification()
    {
        ...
        update_option('bizento_notification_dismissed', true);
    }
            ...
            (function($) {
                $(document).on('click', '.notice.is-dismissible .notice-dismiss', function() {
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'dismiss_bizento_notification',
...
// Instantiate the bizento Admin Info class
new Bizento_Admin_Info();
